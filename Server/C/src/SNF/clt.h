...
struct SNF_Client_t {
  ...
  /// @brief Saves the 36 ( +1 Null character ) character wide Unique User
  /// IDentifier. Used to defined the connected Client
  char UUID[37];
  /// @brief Saves the socket id
  int sock;
  /// @brief Client's connection mode
  SNF_CLT_MODE mode;
  /// @brief Saves the amount of left request to be responded to. (Effective in
  /// Multi-Shot only)
  uint32_t req_count;
...
///
/// @note   **ht_min_Size** isn't (in most cases) the same as the true size of
/// HashTable length \line
/// @note   \line See \ref snf_hashtable_inis() .
///
///
/// @note   For custom additional client information check the
...
///         with a default \ref SNF_CLT::UUID with the value of
...
/// @warning Make sure you closed the socket file discriptor if you have no use
/// for it.
extern void snf_clt_free(SNF_CLT *Client);
/// @brief Searches for a SNF_CLT depending on their last connected Socket FD
...
/// @param uuid The Client's UUID to search with
/// @return SNF_CLT address pointer where it's value could be:
///         * The searched SNF_CLT instance's address.<br>
///         * **NULL** if no SNF_CLT found.<br>
extern SNF_CLT *snf_clt_get_uuid(const char *uuid);
extern char *snf_clt_get_uuid_sockfd(int Sockfd);
/// @brief Checks the existance of a SNF_CLT depending on their socket's file
/// descriptor
/// @param Sockfd The socket descriptor to search with
...
/// @brief Checks the existance of a SNF_CLT depending on their \ref
/// SNF_CLT::UUID
...
/// @param arg new Client's SNF_CLTSCK address ( fetch with snf_clt_get_sockfd )
/// @return **NULL**
/// @warning  Core Function in network.c do not use it elsewhere.
