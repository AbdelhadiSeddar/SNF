...
do
    ...
    then
        build_force=true
    elif [[ ${!i} = "-gt" ]] || [[ ${!i} = "-t" ]] ||[[ ${!i} = "--gdbtest" ]] || [[ ${!i} = "--test" ]]
    then
        if [[ -z "${TESTING}" ]]
        then
    ...
    then
        ...
        echo -e [4]'\t'\-gt'\t'--gdbtest'\t''\t'\<Test\>'\t'Compiles and Executes \<Test\>.c that\'s situated in ./tests/ and runs it using gdb
        echo -e '\t''\t''\t''\t'Main'\t'Compiles and Executes main.c that is considered the Main Test using gdb
